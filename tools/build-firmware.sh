...
# ── Compile ─────────────────────────────────────────────────────────────
# FQBN matches Arduino IDE board "ESP32S3 Dev Module" with USB CDC on
# boot enabled (required for the config tool's serial connection) and
# the Minimal SPIFFS partition scheme (ota_0/ota_1 each ~1.9 MB).
